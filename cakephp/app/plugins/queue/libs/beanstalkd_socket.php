...
class BeanstalkdSocket {
	...
	public function lt() {
		...
		switch ($status) {
			case 'OK':
				...
				$return = array_slice(explode("\n", str_replace("- ", "",$this->_read((integer)strtok(' ')))), 2);	
				//print_r($retun);
				$x = 0;
				foreach ($return as $tube) {
					$status = $this->st($tube);
					debug($status);
	...
	public function listTubes($type) {
		$this->_write('list-tubes');
		$status = strtok($this->_read(), ' ');
		...
		switch ($status) {
			case 'OK':
				...
				$x = 0;
				foreach ($return as $tube) {
					if ($type == substr($tube, 0, 3)){
						$status = $this->st($tube);
						//print_r($status);
						$pause = explode(" ", $status[12]);
						
						if ($pause[1] == 0){
							$return = explode(" ", $status[3]);
							
							
							$watching = explode(" ", $status[9]);
							
							if ($watching != 0) {
								$t[$tube] = array($return, $watching);	
							}
				...
				if (empty($t)) {
					// now there are no good queus 
					return false;
				} else {
					asort($t);
					$k = array_keys($t);
					
					if ( $t[$k[0]][1][1] == 0 AND $type == "img") {
						return false;
